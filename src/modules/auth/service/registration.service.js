...
export const signup = asyncHandler(async (req, res, next) => {
  ...
  if (matchedUser) {
    return next(new Error("phone is registered before"));
  }
  ...
  const user = await userModel.create({
    ...
    phone: generateEncryption({ plainText: phone }),
    gender,
  });
...
export const resendCode = asyncHandler(async (req, res, next) => {
  ...
  if (user) {
      ...
      if (!user.confirmEmail) {
        if (timeDifference > 2) {
          if (user.OTP[0].tryOfResendCode < 5) {
            emailEvent.emit("resendCode", {
              email,
              tryOfResendCode: user.OTP[0].tryOfResendCode,
              user,
              type,
              subject: "confirm Email",
            });
    ...
    if (type == "forgetPassword") {
        ...
        if (user.isForgetPassword) {
          if (timeDifference > 2) {
            if (user.OTP[1]?.tryOfResendCode < 5) {
              emailEvent.emit("resendCode", {
                email,
                tryOfResendCode: user.OTP[1]?.tryOfResendCode,
                user,
                type,
      ...
      return next(
        new Error("your email is not confrimed , please confirm first ")
      );
    }
  }
